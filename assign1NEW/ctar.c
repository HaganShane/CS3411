...
void extractFiles(int argc, char **argv) {
    ...
    /* per instructions: if the deleted flag is 1, don't extract (must be valid) */
    /*if (archiveFile.deleted == 1){
        writeTo = false;
    }
    if (response == 'Y'){
...
void appendFiles(int argc, char **argv) {
    int result, fileSize, fileHead, fileContents, i;
    struct stat st;
    ...
    // if the access yeilds -1, means the file exist, open our argument and append using flag 
    if (result != -1){
        sprintf(output, "Archive '%s' does exist, appended files!\n", argv[2]);
        write(2, output, strlen(output));
        fileHead = open(argv[2], O_WRONLY | O_APPEND, 0644);
        for (i = 3; i < argc; i++){
